...
public class AddItemBookTest {
	...
	/**
	 * Check that trying to add this new item book (login, password, title, kind,
	 * author, nbPages) raises a BadEntry exception and does not change content of
	 * the <i>ISocialNetwork</i>. If OK, the method just returns 0. If not OK,
	 ...
	 * @param testId
	 *            - the test ID that will prefix any error message displayed by this
	 *            method
	 * @param errorMessage
	...
	/**
	 * Check that trying to add this new item book (login, password, title, kind,
	 * author, nbPages) raises a NotMemberException exception and does not change
	 * content of the <i>ISocialNetwork</i>. If OK, the method just returns 0. If
	 * not OK, displays an error message and returns 1.
	 * 
	 * @param sn
	 *            - the <i>ISocialNetwork</i>
	 * @param login
	...
	private static int addItemBookNotMemberExceptionTest(ISocialNetwork sn, String login, String password, String title,
		...
		try {
			...
			if (nbItemsBooks != sn.nbBooks()) {
				System.out.println(
						"Err " + testId + " : NotMemberException was thrown but the number of item books was changed");
				return 1;
			} else {
				return 0;
			}
	...
	/**
	 * Check that trying to add this new item book (login, password, title, kind,
	 * author, nbPages) raises a ItemBookAlreadyExistsException exception and does
	 * not change content of the <i>ISocialNetwork</i>. If OK, the method just
	 * returns 0. If not OK, displays an error message and returns 1.
	 * 
	 * @param sn
	 *            - the <i>ISocialNetwork</i>
	 * @param login
		...
		try {
			...
			if (nbItemsBooks != sn.nbBooks()) {
				System.out.println("Err " + testId
						+ " : ItemBookAlreadyExistsException was thrown but the number of item books was changed");
				return 1;
			} else {
				return 0;
			}
	...
	private static int addItemBookOKTest(ISocialNetwork sn, String login, String password, String title, String kind,
		...
		try {
			...
			if (nbItemBooks + 1 != sn.nbBooks()) {
				System.out.println(
						"Err " + testId + " : the number of item books (" + nbItemBooks + ") was not incremented");
				return 1;
			} else {
				return 0;
			}
	...
	public static TestReport test() {
		...
		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, " ", "1234", "Lord Of The Rings", "heroic fantasy", "J. R. R. Tolkien",
				30, "3.2",
				"addItemBook() doesn't reject logins that don't contain at least one character other than space");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "esevellec", null, "Lord Of The Rings", "heroic fantasy",
				"J. R. R. Tolkien", 30, "3.3", "addItemBook() doesn't reject null password");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "esevellec", "    123 ", "Lord Of The Rings", "heroic fantasy",
				"J. R. R. Tolkien", 30, "3.4",
				"addItemBook() doesn't reject that don't contain at least 4 characters (not taking into account leading or trailing blanks)");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "Erwan", "1234", null, "heroic fantasy", "J. R. R. Tolkien", 30, "3.5",
				"addItemBook() doesn't reject null title");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "esevellec", "1234", "     ", "heroic fantasy", "J. R. R. Tolkien", 30,
				"3.6", "addItemBook() doesn't reject only space title");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "esevellec", "1234", "Lord Of The Rings", null, "J. R. R. Tolkien", 30,
				"3.7", "addItemBook() doesn't reject null kind");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "esevellec", "1234", "Lord Of The Rings", "heroic fantasy", null, 30,
				"3.8", "addItemBook() doesn't reject null author");

		nbTests++;
		nbErrors += addItemBookBadEntryTest(sn, "esevellec", "1234", "Lord Of The Rings", "heroic fantasy",
				"J. R. R. Tolkien", 0, "3.9", "addItemBook() doesn't reject null number of pages");
		...
		nbErrors += addItemBookNotMemberExceptionTest(sn, "Erwan", "1234", "Lord Of The Rings", "heroic fantasy",
				"J. R. R. Tolkien", 30, "4.1", "addItemBook() doesn't reject a bad login");

		nbTests++;
		nbErrors += addItemBookNotMemberExceptionTest(sn, "esevellec", "12345", "Lord Of The Rings", "heroic fantasy",
				"J. R. R. Tolkien", 30, "4.2", "addItemBook() doesn't reject a bad password");

		nbTests++;
		nbErrors += addItemBookOKTest(sn, "esevellec", "1234", "Lord Of The Rings", "heroic fantasy",
